...
function getDateFormat(range: RangeType, date: Date): string {
  if (range === "day") {
...
export async function getDashboardStats(organizerId: string, range: RangeType) {
  ...
  const grouped: Record<string, { revenue: number; attendees: number }> = {};
  ...
  const eventRevenueMap: Record<
    string,
    { title: string; revenue: number; attendees: number }
  > = {};
...
export async function getOrganizerEventStats(organizerId: string) {
  const events = await prisma.event.findMany({
    where: { organizerId },
