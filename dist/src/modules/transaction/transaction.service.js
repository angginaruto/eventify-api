...
// ── helper: hitung total poin aktif user ─────────────────────
async function getActivePoints(userId) {
    const result = await prisma.point.aggregate({
        where: {
            userId,
            status: "ACTIVE",
...
// ── helper: kurangi poin ─────────────────────────────────────
async function redeemPoints(tx, userId, pointsNeeded) {
    const points = await tx.point.findMany({
        where: {
            userId,
            status: "ACTIVE",
...
// ── POST /transactions ────────────────────────────────────────
export async function createTransaction(customerId, input) {
    const { eventId, quantity, couponCode, usePoints } = input;
    // 1. cek event
    const event = await prisma.event.findUnique({ where: { id: eventId } });
    if (!event)
...
export async function getMyTransactions(customerId) {
...
export async function getEventTransactions(eventId, organizerId) {
    const event = await prisma.event.findUnique({ where: { id: eventId } });
    if (!event)
        throw new Error("Event not found");
