...
export async function createPromotion(eventId, organizerId, input) {
...
export async function getEventPromotions(eventId, organizerId) {
    const event = await prisma.event.findUnique({ where: { id: eventId } });
    if (!event)
        throw new Error("Event not found");
    if (event.organizerId !== organizerId)
...
export async function deletePromotion(promotionId, organizerId) {
    ...
    if (promotion.event.organizerId !== organizerId)
        throw new Error("Forbidden");
...
export async function validatePromoCode(code, eventId) {
