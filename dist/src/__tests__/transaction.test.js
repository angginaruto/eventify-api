...
beforeAll(async () => {
    ...
    const eventsRes = await request(app).get("/api/events?status=PUBLISHED&type=paid");
    publishedEventId = eventsRes.body.data[0]?.id;
    // ambil free event
    const freeRes = await request(app).get("/api/events?status=PUBLISHED&type=free");
...
describe("POST /api/transactions", () => {
    ...
    it("should return 400 if quantity exceeds available seats", async () => {
        ...
        const availableSeats = eventRes.body.data.availableSeats - eventRes.body.data.bookedSeats;
